...
(tablet LAYOUT_VARIANT landed in Phase 4: rail, two-column layouts, expanded
charts; the desktop LAYOUT_VARIANT arrives in Phase 5) · — not built yet.

| Feature | Mobile | Tablet | Desktop | Class | Notes / decision ref |
| --- | --- | --- | --- | --- | --- |
| Authentication | ✅ (demo) | ✅ | base | FULL_PARITY (required) | Demo IdP behind real provider interface (F007) |
| Overview dashboard | ✅ | ✅ | base | FULL_PARITY (required) | Same snapshot as all other surfaces |
| Consent management | ✅ | ✅ | base | FULL_PARITY (required) | Enforcement in BFF snapshot: AA withdrawal filters holdings/sources everywhere |
| Financial profile | ✅ | ✅ | base | FULL_PARITY (required) | 5-step mobile form (F109); web may group fields = LAYOUT_VARIANT, same schema |
| Risk profiling | ✅ | ✅ | base | FULL_PARITY (required) | 6-step questionnaire; scoring in domain engine |
| Wealth health score | ✅ | ✅ | base | FULL_PARITY (required) | Pillars + explanations from shared engine |
| Spending behaviour | ✅ | ✅ | base | FULL_PARITY (required) | summarizeSpending in domain engine; Copilot quotes same numbers |
| Portfolio health | ✅ | ✅ | base | FULL_PARITY (required) | Web adds tables (WEB_ENHANCEMENT presentation) |
| Goals + simulations | ✅ | ✅ | base | FULL_PARITY (required) | Creation flow + live simulator via the same projectGoal engine |
| Recommendations | ✅ | ✅ | base | FULL_PARITY (required) | Evidence screen live; same recs on overview/Copilot |
| Copilot conversation | ✅ text | ✅ | base | FULL_PARITY (required) | Deterministic brain on shared snapshot; split view = LAYOUT_VARIANT (F116); voice = CAPABILITY_FALLBACK |
| Advisor handoff | ✅ | ✅ | base | FULL_PARITY (required) | 2-step request; reference id; context-sharing note |
